(function () {
    ...
    function BillingCycleController($http, $location, msgs, tabs, consts) {
        ...
        function refresh() {
            ...
            $http.get(`${consts.apiUrl}/billingCycles?skip=${(page - 1) * 10}&limit=10`)
                .then((response) => {
                    vm.billingCycle = { credits: [{}], debts: [{}] }
                    vm.billingCycles = response.data;
                    calculateValues()

                    $http.get(`${consts.apiUrl}/billingCycles/count`)
                        .then((response) => {
        ...
        function create() {
            $http.post(`${consts.apiUrl}/billingCycles`, vm.billingCycle)
                .then((response) => {
                    refresh();
                    msgs.addSuccess(SUCESS_MESSAGE)
                })
        ...
        function remove() {
            const resource = `${consts.apiUrl}/billingCycles/${vm.billingCycle._id}`
